(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('json'), require('top-domain')) :
	typeof define === 'function' && define.amd ? define(['json', 'top-domain'], factory) :
	(global.rakam = factory(global.JSON,global.topDomain));
}(this, (function (JSON,topDomain) { 'use strict';

JSON = JSON && JSON.hasOwnProperty('default') ? JSON['default'] : JSON;
topDomain = topDomain && topDomain.hasOwnProperty('default') ? topDomain['default'] : topDomain;
...
User.prototype.set = function (properties, callback) {
    new xhr(getUrl(this.options) + "/set_properties", {
        api: {
...
User.prototype._merge = function (deviceId, createdAt, callback) {
    new xhr(getUrl(this.options) + "/merge", {
        api: {
            "api_version": API_VERSION$1,
            "api_key": this.options.apiKey,
...
User.prototype.setOnce = function (properties, callback) {
    new xhr(getUrl(this.options) + "/set_properties_once", {
        api: {
            "api_version": API_VERSION$1,
            "api_key": this.options.apiKey
        },
...
User.prototype.increment = function (property, value, callback) {
    new xhr(getUrl(this.options) + "/increment_property", {
        api: {
            "api_version": API_VERSION$1,
            "api_key": this.options.apiKey
        },
...
User.prototype.unset = function (properties, callback) {
    new xhr(getUrl(this.options) + "/unset_properties", {
        api: {
            "api_version": API_VERSION$1,
            "api_key": this.options.apiKey
        },
        id: this.options.userId,
        properties: type$1(properties) === "array" ? properties : [properties]
...
Rakam._getUtmData = function (rawCookie, query) {
    ...
    var cookie$$1 = rawCookie ? '?' + rawCookie.split('.').slice(-1)[0].replace(/\|/g, '&') : '';

    var fetchParam = function (queryName, query, cookieName, cookie$$1) {
        return Rakam._getUtmParam(queryName, query) || Rakam._getUtmParam(cookieName, cookie$$1);
    };

    return {
        utm_source: fetchParam('utm_source', query, 'utmcsr', cookie$$1),
        utm_medium: fetchParam('utm_medium', query, 'utmcmd', cookie$$1),
        utm_campaign: fetchParam('utm_campaign', query, 'utmccn', cookie$$1),
        utm_term: fetchParam('utm_term', query, 'utmctr', cookie$$1),
        utm_content: fetchParam('utm_content', query, 'utmcct', cookie$$1)
    };
};
